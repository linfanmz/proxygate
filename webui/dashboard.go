...
:root{
  ...
  --shadow-sm:0 4px 12px rgba(0,0,0,0.06),0 1px 2px rgba(0,0,0,0.04);
  --shadow-md:0 10px 24px rgba(0,0,0,0.08),0 2px 8px rgba(0,0,0,0.04);
  --shadow-lg:0 20px 48px rgba(0,0,0,0.1),0 4px 16px rgba(0,0,0,0.04);
  --radius-sm:8px;
  --radius-md:10px;
  --radius-lg:12px;
  --radius-xl:14px;
...
.panel-label.accent{color:var(--fg-dim)}
.health-grid{
  display:grid;
...
.quality-s{background:var(--primary)}
.quality-a{background:#6366f1}
.quality-b{background:#f59e0b}
...
.upload-dropzone:hover{background:var(--primary-soft);border-color:var(--primary)}
...
.subscription-badge-warm{background:var(--primary-soft);color:var(--primary)}
    ...
    <aside class="sidebar">
      ...
      <div class="quality-bar">
        ...
        <div class="quality-legend">
          ...
          <div class="quality-legend-item"><span class="quality-legend-dot" style="background:#6366f1"></span><span data-i18n="quality.grade_a">A级</span> (<span id="grade-a-count">0</span>)</div>
          <div class="quality-legend-item"><span class="quality-legend-dot" style="background:#f59e0b"></span><span data-i18n="quality.grade_b">B级</span> (<span id="grade-b-count">0</span>)</div>
...
<div class="modal-overlay" id="settings-modal" onclick="if(event.target===this) closeSettings()">
  <div class="modal">
    ...
    <!-- 代理认证 -->
    <div class="form-section">
      <div class="form-section-title">代理认证 Proxy Auth</div>
      <div class="form-grid">
        <div class="form-group" style="grid-column:1/-1">
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer">
            <input type="checkbox" id="cfg-proxy-auth-enabled" onchange="toggleProxyAuthFields()" style="width:auto;min-height:auto">
            <span>启用代理认证 (公网部署建议开启)</span>
          </label>
        </div>
        <div class="form-group">
          <label>认证用户名</label>
          <input type="text" id="cfg-proxy-auth-username" placeholder="proxy">
        </div>
        <div class="form-group">
          <label>认证密码 (留空=不修改)</label>
          <input type="password" id="cfg-proxy-auth-password" placeholder="留空则保持现有密码">
        </div>
        <div class="form-group" style="grid-column:1/-1">
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer">
            <input type="checkbox" id="cfg-local-auth-bypass" style="width:auto;min-height:auto">
            <span>允许本地回环地址跳过认证</span>
          </label>
        </div>
      </div>
    </div>
    ...
    <!-- 代理用户管理 -->
    <div class="form-section">
      <div class="form-section-title">代理认证用户管理</div>
      <div id="user-list" style="margin-bottom:14px;max-height:150px;overflow-y:auto"></div>
      <div class="form-grid">
        <div class="form-group">
          <input type="text" id="new-user-name" placeholder="用户名">
        </div>
        <div class="form-group">
          <input type="password" id="new-user-password" placeholder="密码 (至少4位)">
        </div>
      </div>
      <div style="margin-top:10px">
        <button class="ctrl-btn-primary" onclick="addProxyUser()" style="flex:none;min-width:80px">添加用户</button>
      </div>
    </div>

    <!-- WebUI 密码修改 -->
    <div class="form-section">
      <div class="form-section-title">修改管理密码</div>
      <div class="form-grid">
        <div class="form-group">
          <label>当前密码</label>
          <input type="password" id="cfg-current-password">
        </div>
        <div class="form-group">
          <label>新密码 (至少6位)</label>
          <input type="password" id="cfg-new-password" minlength="6">
        </div>
      </div>
    </div>
...
async function openSettings() {
  ...
  document.getElementById('cfg-validate-url').value = cfg.validate_url || '';

  // 代理认证配置
  document.getElementById('cfg-proxy-auth-enabled').checked = cfg.proxy_auth_enabled || false;
  document.getElementById('cfg-proxy-auth-username').value = cfg.proxy_auth_username || 'proxy';
  document.getElementById('cfg-proxy-auth-password').value = '';
  document.getElementById('cfg-local-auth-bypass').checked = cfg.local_auth_bypass || false;
...
function toggleProxyAuthFields() {
  const enabled = document.getElementById('cfg-proxy-auth-enabled').checked;
  const fields = ['cfg-proxy-auth-username', 'cfg-proxy-auth-password', 'cfg-local-auth-bypass'];
  fields.forEach(id => {
...
async function saveConfig() {
  const cfg = {
    ...
    validate_url: document.getElementById('cfg-validate-url').value.trim(),
    ...
    proxy_auth_enabled: document.getElementById('cfg-proxy-auth-enabled').checked,
    proxy_auth_username: document.getElementById('cfg-proxy-auth-username').value.trim(),
    proxy_auth_password: document.getElementById('cfg-proxy-auth-password').value,
    local_auth_bypass: document.getElementById('cfg-local-auth-bypass').checked,
    custom_proxy_mode: (() => {
  ...
  if (newPassword && newPassword.length >= 6) {
    ...
    const pwResult = await api('/api/config/change-password', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({current_password: currentPassword, new_password: newPassword})
    });
    if (!pwResult || pwResult.error) {
      alert('密码修改失败: ' + (pwResult ? pwResult.error : 'unknown'));
      return;
    }
  }
...
async function loadUsers() {
  ...
  if (!el) return;
  if (users.length === 0) {
    el.innerHTML = '<div style="color:var(--fg-soft);font-size:12px;padding:8px">暂无用户（使用配置中的单用户认证）</div>';
    return;
  }
  el.innerHTML = users.map(u =>
    '<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border)">' +
      '<span style="font-size:13px">' + u.username + (u.enabled ? '' : ' <span style="color:var(--red)">(已禁用)</span>') + '</span>' +
      '<span>' +
        '<button onclick="toggleProxyUser(' + u.id + ')" style="background:none;border:1px solid var(--border);border-radius:4px;cursor:pointer;font-size:11px;margin-right:4px;padding:2px 8px">' + (u.enabled ? '禁用' : '启用') + '</button>' +
        '<button onclick="deleteProxyUser(' + u.id + ')" style="background:none;border:1px solid var(--red);color:var(--red);border-radius:4px;cursor:pointer;font-size:11px;padding:2px 8px">删除</button>' +
      '</span>' +
    '</div>'
  ).join('');
}

async function addProxyUser() {
...
async function toggleProxyUser(id) {
