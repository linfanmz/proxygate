...
func (s *Server) Start() error {
	...
	go func() {
		for {
			...
			for ip, state := range loginFailures {
				if now.After(state.lockedUntil) && state.count == 0 {
	...
	mux.HandleFunc("/api/config/change-password", s.authMiddleware(s.apiChangePassword))

	// 池子历史
	mux.HandleFunc("/api/pool/history", s.readOnlyMiddleware(s.apiPoolHistory))

	// 备份导出
	mux.HandleFunc("/api/backup", s.authMiddleware(s.apiBackup))
	mux.HandleFunc("/api/export-proxies", s.readOnlyMiddleware(s.apiExportProxies))

	// 审计日志
	mux.HandleFunc("/api/audit", s.authMiddleware(s.apiAudit))

	// 代理用户管理 API
	mux.HandleFunc("/api/users", s.readOnlyMiddleware(s.apiUsers))
	mux.HandleFunc("/api/users/add", s.authMiddleware(s.apiUserAdd))
	mux.HandleFunc("/api/users/delete", s.authMiddleware(s.apiUserDelete))
	mux.HandleFunc("/api/users/toggle", s.authMiddleware(s.apiUserToggle))

	// 代理源管理 API
	mux.HandleFunc("/api/sources", s.readOnlyMiddleware(s.apiSources))
	mux.HandleFunc("/api/sources/add", s.authMiddleware(s.apiSourceAdd))
	mux.HandleFunc("/api/sources/toggle", s.authMiddleware(s.apiSourceToggle))
	mux.HandleFunc("/api/sources/delete", s.authMiddleware(s.apiSourceDelete))

	// API Key 管理
	mux.HandleFunc("/api/keys", s.authMiddleware(s.apiKeys))
	mux.HandleFunc("/api/keys/add", s.authMiddleware(s.apiKeyAdd))
	mux.HandleFunc("/api/keys/toggle", s.authMiddleware(s.apiKeyToggle))
	mux.HandleFunc("/api/keys/delete", s.authMiddleware(s.apiKeyDelete))
...
// authMiddleware 管理员权限中间件（必须登录 + body 大小限制）
...
func (s *Server) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// 暴力破解保护
	clientIP := requestClientIP(r.RemoteAddr)
	now := time.Now()
	loginFailuresMu.Lock()
	if state, ok := loginFailures[clientIP]; ok && now.Before(state.lockedUntil) {
		loginFailuresMu.Unlock()
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
	...
	if subtle.ConstantTimeCompare([]byte(hash), []byte(s.cfg.WebUIPasswordHash)) != 1 {
		loginFailuresMu.Lock()
		state := loginFailures[clientIP]
		state.count++
		if state.count >= maxLoginAttempts {
			state.lockedUntil = now.Add(loginLockoutMinutes * time.Minute)
			state.count = 0
		}
		loginFailures[clientIP] = state
		loginFailuresMu.Unlock()
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		fmt.Fprint(w, loginHTMLWithError)
		return
...
func (s *Server) apiConfig(w http.ResponseWriter, r *http.Request) {
	...
	jsonOK(w, map[string]interface{}{
		...
		"proxy_auth_has_password": cfg.ProxyAuthPassword != "",
		"local_auth_bypass":   cfg.LocalAuthBypass,
...
func (s *Server) apiConfigSave(w http.ResponseWriter, r *http.Request) {
	...
	var req struct {
		...
		ProxyAuthPassword     string   `json:"proxy_auth_password"` // 空字符串表示不修改
		LocalAuthBypass       *bool    `json:"local_auth_bypass"`
	...
	if req.ProxyAuthEnabled != nil {
		newCfg.ProxyAuthEnabled = *req.ProxyAuthEnabled
	}
	if req.ProxyAuthUsername != "" {
		newCfg.ProxyAuthUsername = req.ProxyAuthUsername
	}
	...
	if req.LocalAuthBypass != nil {
		newCfg.LocalAuthBypass = *req.LocalAuthBypass
	}
	if req.ValidateURL != "" {
		newCfg.ValidateURL = req.ValidateURL
	}
	...
	s.recordAudit(r, "config_save", fmt.Sprintf("pool=%d http_ratio=%.2f", req.PoolMaxSize, req.PoolHTTPRatio))
...
// apiChangePassword 修改 WebUI 登录密码
func (s *Server) apiChangePassword(w http.ResponseWriter, r *http.Request) {
	...
	cfg := s.currentConfig()
	if subtle.ConstantTimeCompare([]byte(currentHash), []byte(cfg.WebUIPasswordHash)) != 1 {
	...
	newCfg := *cfg
	newCfg.WebUIPasswordHash = newHash
	if err := config.Save(&newCfg); err != nil {
		jsonError(w, "save config error: "+err.Error(), http.StatusInternalServerError)
		return
	}

	// 密码修改后清除所有现有 session，强制重新登录
	sessionsMu.Lock()
	...
	log.Printf("[config] WebUI 密码已更新，所有 session 已失效")
	jsonOK(w, map[string]string{"status": "password changed"})
}

// apiPoolHistory 获取池子历史
func (s *Server) apiPoolHistory(w http.ResponseWriter, r *http.Request) {
	entries, err := s.storage.GetPoolHistory(288)
	if err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	if entries == nil {
		entries = []storage.PoolHistoryEntry{}
	}
	jsonOK(w, entries)
}
...
func (s *Server) apiBackup(w http.ResponseWriter, r *http.Request) {
	db := s.storage.GetDB()
	if db == nil {
		jsonError(w, "database not available", http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", "application/octet-stream")
	w.Header().Set("Content-Disposition", `attachment; filename="proxygate-backup.db"`)
	// SQLite backup via .dump would be ideal, but for simplicity stream the raw file
...
// apiExportProxies 导出代理列表
func (s *Server) apiExportProxies(w http.ResponseWriter, r *http.Request) {
	...
	case "csv":
		...
		fmt.Fprintln(w, "address,protocol,exit_ip,exit_location,latency,quality_grade,source")
		for _, p := range proxies {
			fmt.Fprintf(w, "%s,%s,%s,%s,%d,%s,%s\n",
				p.Address, p.Protocol, p.ExitIP, p.ExitLocation, p.Latency, p.QualityGrade, p.Source)
		}
	default:
		jsonOK(w, proxies)
	}
...
func (s *Server) apiUsers(w http.ResponseWriter, r *http.Request) {
	users, err := s.storage.GetProxyUsers()
	if err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	if users == nil {
		users = []storage.ProxyUser{}
	}
	jsonOK(w, users)
}
...
func (s *Server) apiUserAdd(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.storage.AddProxyUser(req.Username, passwordHash); err != nil {
		jsonError(w, "add user error: "+err.Error(), http.StatusInternalServerError)
		return
	}
	log.Printf("[webui] 添加代理用户: %s", req.Username)
	jsonOK(w, map[string]string{"status": "added"})
}

// apiUserDelete 删除代理用户
func (s *Server) apiUserDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.storage.DeleteProxyUser(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	jsonOK(w, map[string]string{"status": "deleted"})
...
func (s *Server) apiUserToggle(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.storage.ToggleProxyUser(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *Server) apiAudit(w http.ResponseWriter, r *http.Request) {
	events, err := s.storage.GetAuditEvents(100)
	if err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *Server) recordAudit(r *http.Request, action, details string) {
	actor := requestClientIP(r.RemoteAddr)
	if cookie, err := r.Cookie("session"); err == nil {
		actor = "admin:" + maskGuestAddress(actor)
		_ = cookie
	}
	s.storage.RecordAudit(actor, action, details)
}

// ========== API Key 管理 API ==========

func (s *Server) apiKeys(w http.ResponseWriter, r *http.Request) {
	keys, err := s.storage.GetApiKeys()
	if err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *Server) apiKeyAdd(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		jsonError(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if req.Scope != "read" && req.Scope != "full" {
		req.Scope = "read"
	}
	...
	if err := s.storage.AddApiKey(req.Name, req.Scope, keyHash); err != nil {
		jsonError(w, "add key error: "+err.Error(), http.StatusInternalServerError)
		return
	}
	log.Printf("[webui] 添加 API Key: %s (scope=%s)", req.Name, req.Scope)
	jsonOK(w, map[string]interface{}{"status": "added", "key": rawKey})
}

func (s *Server) apiKeyToggle(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		jsonError(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if err := s.storage.ToggleApiKey(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	jsonOK(w, map[string]string{"status": "toggled"})
...
func (s *Server) apiKeyDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.storage.DeleteApiKey(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	jsonOK(w, map[string]string{"status": "deleted"})
...
func (s *Server) apiSourceAdd(w http.ResponseWriter, r *http.Request) {
	...
	if req.Protocol != "http" && req.Protocol != "socks5" {
		jsonError(w, "protocol must be http or socks5", http.StatusBadRequest)
		return
	}
	if req.Category != "fast" && req.Category != "slow" {
		req.Category = "fast"
	}
	if err := s.storage.AddSource(req.URL, req.Protocol, req.Category); err != nil {
		jsonError(w, "add source error: "+err.Error(), http.StatusInternalServerError)
		return
	}
	log.Printf("[webui] 添加代理源: %s (%s/%s)", req.URL, req.Protocol, req.Category)
	jsonOK(w, map[string]string{"status": "added"})
}

// apiSourceToggle 切换代理源启用状态
func (s *Server) apiSourceToggle(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		jsonError(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if err := s.storage.ToggleSource(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	jsonOK(w, map[string]string{"status": "toggled"})
...
func (s *Server) apiSourceDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.storage.DeleteSource(req.ID); err != nil {
		jsonError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	log.Printf("[webui] 删除代理源 #%d", req.ID)
	jsonOK(w, map[string]string{"status": "deleted"})
}
