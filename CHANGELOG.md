...
- **全配置 WebUI 化**：代理认证设置（开关/用户名/密码/本地绕行）、管理密码修改、验证 URL 等所有配置项均可在 WebUI 设置面板中操作，无需编辑 `.env` 文件
- **多用户代理认证**：支持创建多个代理认证用户（用户名/密码），可在 WebUI 中增删改查，HTTP 和 SOCKS5 代理均支持
- **代理源管理**：30+ 代理抓取源从硬编码迁移到 SQLite 数据库，支持 WebUI 动态添加/启用/禁用/删除源，无需重新编译
- **API Key 管理**：支持生成 API Key（read/full 权限），供外部程序调用 API
- **审计日志**：自动记录所有管理员操作（配置修改、用户管理、源管理等），可通过 API 查询
- **池子历史记录**：每 5 分钟自动记录池子状态快照，支持趋势分析
- **客户端速率限制**：代理端口内置 per-IP 速率限制（2 req/s，突发 10），防止池子被单一客户端耗尽
- **登录暴力破解保护**：WebUI 登录 5 次失败后 IP 锁定 5 分钟
- **健康检查端点**：新增 `/health` 和 `/ready` 端点，适用于 Docker/K8s 容器编排
- **代理导出**：支持 JSON/CSV/TXT 格式导出代理列表
- **数据库备份下载**：管理员可通过 API 下载 SQLite 数据库备份
- **WebUI 全新商务主题**：浅色专业风格，蓝白配色，Inter 字体

### 修复

- **竞态条件**：`TryAddProxy` 添加互斥锁，消除并发入池时的 slot 超限问题
- **Goroutine 泄漏**：隧道中继连接添加 5 分钟空闲超时，防止对端失联导致协程永久阻塞
- **定时器泄漏**：订阅探测循环中 `time.After` 替换为 `time.NewTicker`
- **请求体无界读取**：HTTP 代理请求体读取添加 10MB 上限
- **管理员端点无体大小限制**：所有管理员 POST 端点添加 256KB 体大小限制
- **请求重放逻辑**：收紧为仅 GET/HEAD/OPTIONS/TRACE 可重试
- **会话安全**：管理员密码修改后自动清除所有现有 session
- **后台协程安全关闭**：WebUI 后台任务通过 WaitGroup 追踪，防止关闭后访问数据库

### 变更

- 默认 HTTP 代理占比从 50% 调整为 30%（更多槽位给 SOCKS5）
- 标准延迟阈值从 2000ms 放宽到 2500ms
- 验证超时从 8 秒增加到 10 秒
- WebUI 主题从暖色调转为浅色商务风格
