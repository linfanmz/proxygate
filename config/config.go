...
func Load() *Config {
	...
	if err == nil {
		...
		if json.Unmarshal(data, &saved) == nil {
			...
			cfg.LocalAuthBypass = saved.LocalAuthBypass

			// WebUI 密码（config.json 优先于环境变量）
...
type savedConfig struct {
	...
	LocalAuthBypass       bool   `json:"local_auth_bypass,omitempty"`
...
func Save(cfg *Config) error {
	...
	data, err := json.MarshalIndent(savedConfig{
		...
		ProxyAuthEnabled:      snapshot.ProxyAuthEnabled,
		ProxyAuthUsername:     snapshot.ProxyAuthUsername,
		ProxyAuthPassword:     snapshot.ProxyAuthPassword,
		ProxyAuthPasswordHash: snapshot.ProxyAuthPasswordHash,
		LocalAuthBypass:       snapshot.LocalAuthBypass,
		WebUIPasswordHash:     snapshot.WebUIPasswordHash,
