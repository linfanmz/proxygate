...
管理员登录后进入系统设置 → 地理过滤，可直接设置：
- **允许国家（白名单）**：输入国家代码如 `US,JP,KR,SG`
- **屏蔽国家（黑名单）**：输入国家代码如 `CN,RU,KP`

配置实时保存到 `config.json`，无需重启。
...
| 模式 | 配置方式 | 说明 |
|------|---------|------|
| 黑名单 | WebUI / ENV `BLOCKED_COUNTRIES` | 屏蔽指定国家，其余放行 |
| 白名单 | WebUI / ENV `ALLOWED_COUNTRIES` | 仅允许指定国家，其余拒绝 |

> **优先级**：白名单非空时生效，黑名单被忽略。WebUI 配置优先于环境变量。
