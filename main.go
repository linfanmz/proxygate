...
func main() {
	...
	fetch := fetcher.New(cfg.HTTPSourceURL, cfg.SOCKS5SourceURL, sourceMgr, store)
...
func startStatusMonitor(ctx context.Context, poolMgr *pool.Manager, fetch *fetcher.Fetcher, validate *validator.Validator, store *storage.Storage) {
	...
	for {
		...
		case <-ticker.C:
			...
			// 每5分钟记录一次池子历史
			if ticks%10 == 0 {
				dist, _ := store.GetQualityDistribution()
				if dist == nil {
					dist = map[string]int{}
				}
				store.RecordPoolHistory(status.Total, status.HTTP, status.SOCKS5, status.CustomCount, status.State, dist)
			}
