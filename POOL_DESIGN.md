...
### proxy_users 表（v0.5.0+）

| 字段 | 类型 | 说明 |
|------|------|------|
| username | TEXT UNIQUE | 代理认证用户名 |
| password | TEXT | SHA256 密码哈希 |
| enabled | INTEGER | 是否启用 |

### proxy_sources 表（v0.5.0+）

| 字段 | 类型 | 说明 |
|------|------|------|
| url | TEXT UNIQUE | 代理列表源 URL |
| protocol | TEXT | http / socks5 |
| category | TEXT | fast / slow |
| enabled | INTEGER | 是否启用 |

### pool_history 表（v0.5.0+）

每 5 分钟自动记录池子状态快照，用于趋势分析。

### api_keys 表（v0.5.0+）

外部 API 访问密钥，支持 read/full 权限范围。

### audit_events 表（v0.5.0+）

管理员操作审计日志（配置修改、用户管理、源管理等）。
...
`TryAddProxy` 使用 `sync.Mutex` 序列化免费代理的 slot 检查与写入操作，防止 HTTP 和 SOCKS5 双协程并发入池时超出容量限制。订阅代理（source=custom）不受 slot 限制，直接入池无需加锁。

入池策略：
1. 协议槽位未满 → 直接入池
2. 槽位满但总量允许 10% 浮动 → 浮动入池
3. 全部满 → 尝试替换延迟最高的代理（新代理需快 30%）
