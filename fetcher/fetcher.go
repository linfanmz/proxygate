...
func New(httpURL, socks5URL string, sourceManager *SourceManager, store *storage.Storage) *Fetcher {
	return &Fetcher{
		store:         store,
		sourceManager: sourceManager,
		client: &http.Client{
...
// getSources 从数据库读取代理源，若无则回退到硬编码
func (f *Fetcher) getSources(category string) []Source {
	if f.store != nil {
		srcs, err := f.store.GetEnabledSourcesByCategory(category)
		if err == nil && len(srcs) > 0 {
			result := make([]Source, 0, len(srcs))
			for _, s := range srcs {
				result = append(result, Source{URL: s.URL, Protocol: s.Protocol})
			}
			return result
		}
	}
	// 回退到硬编码
	if category == "fast" {
		return fastUpdateSources
	}
	return slowUpdateSources
}

// getAllSources 从DB读取所有已启用源
func (f *Fetcher) getAllSources() []Source {
	if f.store != nil {
		srcs, err := f.store.GetAllEnabledSources()
		if err == nil && len(srcs) > 0 {
			result := make([]Source, 0, len(srcs))
			for _, s := range srcs {
				result = append(result, Source{URL: s.URL, Protocol: s.Protocol})
...
func (f *Fetcher) FetchSmart(mode string, preferredProtocol string) ([]storage.Proxy, error) {
	...
	case "emergency":
		...
		sources = f.filterAvailableSources(f.getAllSources(), preferredProtocol, true)
		log.Printf("[fetch] 🚨 紧急模式: 使用 %d 个源（忽略断路器）", len(sources))

	case "refill":
		// 补充模式：使用快更新源
		sources = f.filterAvailableSources(f.getSources("fast"), preferredProtocol, false)
		log.Printf("[fetch] 🔄 补充模式: 使用 %d 个快更新源", len(sources))

	case "optimize":
		// 优化模式：随机选择2-3个慢更新源
		sources = f.selectRandomSources(f.getSources("slow"), 3, preferredProtocol)
