...
func (s *Storage) initSchema() error {
	...
	// 种子默认源（仅表为空时）
	var sourceCount int
	s.db.QueryRow(`SELECT COUNT(*) FROM proxy_sources`).Scan(&sourceCount)
	if sourceCount == 0 {
		s.seedDefaultSources()
	}
	...
	_, err = s.db.Exec(`
		CREATE TABLE IF NOT EXISTS pool_history (
			...
			socks5     INTEGER NOT NULL DEFAULT 0,
			state      TEXT NOT NULL DEFAULT '',
			custom_count INTEGER NOT NULL DEFAULT 0,
			grade_s    INTEGER NOT NULL DEFAULT 0,
...
// RecordAudit 记录审计事件
func (s *Storage) RecordAudit(actor, action, details string) error {
	_, err := s.db.Exec(`INSERT INTO audit_events (actor, action, details) VALUES (?, ?, ?)`, actor, action, details)
	return err
}

// GetAuditEvents 获取最近的审计事件
func (s *Storage) GetAuditEvents(limit int) ([]AuditEvent, error) {
	if limit <= 0 {
		limit = 100
	}
	rows, err := s.db.Query(`SELECT timestamp, actor, action, details FROM audit_events ORDER BY id DESC LIMIT ?`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var e AuditEvent
		var ts string
		if err := rows.Scan(&ts, &e.Actor, &e.Action, &e.Details); err != nil {
			return nil, err
		}
		e.Timestamp = ts
...
// GetApiKeys 获取所有API Key（不含哈希）
func (s *Storage) GetApiKeys() ([]ApiKey, error) {
	rows, err := s.db.Query(`SELECT id, name, scope, enabled, created_at, COALESCE(last_used,'') FROM api_keys ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var k ApiKey
		var enabled int
		if err := rows.Scan(&k.ID, &k.Name, &k.Scope, &enabled, &k.CreatedAt, &k.LastUsed); err != nil {
			return nil, err
		}
		k.Enabled = enabled == 1
		keys = append(keys, k)
...
// AddApiKey 添加API Key，返回原始key（仅此一次可见）
func (s *Storage) AddApiKey(name, scope, keyHash string) error {
	_, err := s.db.Exec(`INSERT INTO api_keys (key_hash, name, scope) VALUES (?, ?, ?)`, keyHash, name, scope)
	return err
}

// VerifyApiKey 验证API Key，返回scope
func (s *Storage) VerifyApiKey(keyHash string) (string, bool, error) {
	var scope string
	var enabled int
	err := s.db.QueryRow(`SELECT scope, enabled FROM api_keys WHERE key_hash = ?`, keyHash).Scan(&scope, &enabled)
	if err != nil {
		return "", false, err
	}
	if enabled != 1 {
...
// ToggleApiKey 切换API Key启用状态
func (s *Storage) ToggleApiKey(id int64) error {
	_, err := s.db.Exec(`UPDATE api_keys SET enabled = CASE WHEN enabled = 1 THEN 0 ELSE 1 END WHERE id = ?`, id)
	return err
}
...
// RecordPoolHistory 记录池子状态快照
func (s *Storage) RecordPoolHistory(total, http, socks5, customCount int, state string, grades map[string]int) error {
	_, err := s.db.Exec(
		`INSERT INTO pool_history (total, http, socks5, state, custom_count, grade_s, grade_a, grade_b, grade_c)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		total, http, socks5, state, customCount,
		grades["S"], grades["A"], grades["B"], grades["C"],
	)
	return err
}

// PoolHistoryEntry 池子历史记录
...
// GetPoolHistory 获取最近N条池子历史记录
func (s *Storage) GetPoolHistory(limit int) ([]PoolHistoryEntry, error) {
	if limit <= 0 {
		limit = 288 // 24h at 5min intervals
	}
	rows, err := s.db.Query(
		`SELECT timestamp, total, http, socks5, state, custom_count
		 FROM pool_history ORDER BY id DESC LIMIT ?`, limit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var entries []PoolHistoryEntry
	for rows.Next() {
		var e PoolHistoryEntry
		var ts string
		if err := rows.Scan(&ts, &e.Total, &e.HTTP, &e.SOCKS5, &e.State, &e.CustomCount); err != nil {
			return nil, err
		}
		e.Timestamp = ts
...
// seedDefaultSources 种子默认代理源
func (s *Storage) seedDefaultSources() {
	defaults := []struct {
		url, protocol, category string
	}{
		// 快更新源
		{"https://raw.githubusercontent.com/ProxyScraper/ProxyScraper/main/http.txt", "http", "fast"},
		{"https://raw.githubusercontent.com/ProxyScraper/ProxyScraper/main/socks4.txt", "socks5", "fast"},
		{"https://raw.githubusercontent.com/ProxyScraper/ProxyScraper/main/socks5.txt", "socks5", "fast"},
		{"https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/http.txt", "http", "fast"},
		{"https://raw.githubusercontent.com/prxchk/proxy-list/main/http.txt", "http", "fast"},
		{"https://raw.githubusercontent.com/prxchk/proxy-list/main/socks5.txt", "socks5", "fast"},
		{"https://raw.githubusercontent.com/prxchk/proxy-list/main/socks4.txt", "socks5", "fast"},
		{"https://cdn.jsdelivr.net/gh/sunny9577/proxy-scraper/generated/http_proxies.txt", "http", "fast"},
		{"https://cdn.jsdelivr.net/gh/sunny9577/proxy-scraper/generated/socks5_proxies.txt", "socks5", "fast"},
		{"https://cdn.jsdelivr.net/gh/sunny9577/proxy-scraper/generated/socks4_proxies.txt", "socks5", "fast"},
		// 慢更新源
		{"https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/http.txt", "http", "slow"},
		{"https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks4.txt", "socks5", "slow"},
		{"https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks5.txt", "socks5", "slow"},
		{"https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/socks4.txt", "socks5", "slow"},
		{"https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/socks5.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/databay-labs/free-proxy-list/http.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/databay-labs/free-proxy-list/socks5.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/Anonym0usWork1221/Free-Proxies/proxy_files/http_proxies.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/Anonym0usWork1221/Free-Proxies/proxy_files/socks5_proxies.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/Anonym0usWork1221/Free-Proxies/proxy_files/socks4_proxies.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/ALIILAPRO/Proxy/http.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/ALIILAPRO/Proxy/socks4.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/vakhov/fresh-proxy-list/http.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/vakhov/fresh-proxy-list/socks5.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/vakhov/fresh-proxy-list/socks4.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/Zaeem20/FREE_PROXIES_LIST/http.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/Zaeem20/FREE_PROXIES_LIST/socks4.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/hookzof/socks5_list/proxy.txt", "socks5", "slow"},
		{"https://cdn.jsdelivr.net/gh/proxy4parsing/proxy-list/http.txt", "http", "slow"},
		{"https://cdn.jsdelivr.net/gh/proxy4parsing/proxy-list/socks5.txt", "socks5", "slow"},
	}
	...
	stmt, err := tx.Prepare(`INSERT OR IGNORE INTO proxy_sources (url, protocol, category) VALUES (?, ?, ?)`)
	if err != nil {
		tx.Rollback()
		return
	}
	...
	for _, src := range defaults {
		stmt.Exec(src.url, src.protocol, src.category)
	}
	tx.Commit()
}

// GetSources 获取所有代理源
func (s *Storage) GetSources() ([]ProxySource, error) {
	rows, err := s.db.Query(`SELECT id, url, protocol, category, enabled FROM proxy_sources ORDER BY category, id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var src ProxySource
		var enabled int
		if err := rows.Scan(&src.ID, &src.URL, &src.Protocol, &src.Category, &enabled); err != nil {
			return nil, err
		}
...
// GetEnabledSourcesByCategory 获取指定分类的已启用源
func (s *Storage) GetEnabledSourcesByCategory(category string) ([]ProxySource, error) {
	rows, err := s.db.Query(
		`SELECT id, url, protocol, category, enabled FROM proxy_sources WHERE enabled = 1 AND category = ? ORDER BY id`,
		category,
	)
	if err != nil {
		return nil, err
...
// GetAllEnabledSources 获取所有已启用的源
func (s *Storage) GetAllEnabledSources() ([]ProxySource, error) {
	rows, err := s.db.Query(
		`SELECT id, url, protocol, category, enabled FROM proxy_sources WHERE enabled = 1 ORDER BY category, id`,
	)
	if err != nil {
		return nil, err
	}
...
// AddSource 添加代理源
func (s *Storage) AddSource(url, protocol, category string) error {
	_, err := s.db.Exec(
		`INSERT OR IGNORE INTO proxy_sources (url, protocol, category) VALUES (?, ?, ?)`,
...
func (s *Storage) ToggleSource(id int64) error {
	_, err := s.db.Exec(`UPDATE proxy_sources SET enabled = CASE WHEN enabled = 1 THEN 0 ELSE 1 END WHERE id = ?`, id)
	return err
}

// DeleteSource 删除代理源
func (s *Storage) DeleteSource(id int64) error {
	_, err := s.db.Exec(`DELETE FROM proxy_sources WHERE id = ?`, id)
	return err
}
...
// GetProxyUsers 获取所有代理用户（不含密码）
func (s *Storage) GetProxyUsers() ([]ProxyUser, error) {
	rows, err := s.db.Query(`SELECT id, username, enabled, created_at FROM proxy_users ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.Username, &enabled, &createdAt); err != nil {
			return nil, err
		}
		u.Enabled = enabled == 1
...
// AddProxyUser 添加代理用户
func (s *Storage) AddProxyUser(username, passwordHash string) error {
	_, err := s.db.Exec(`INSERT INTO proxy_users (username, password) VALUES (?, ?)`, username, passwordHash)
	return err
}

// DeleteProxyUser 删除代理用户
func (s *Storage) DeleteProxyUser(id int64) error {
	_, err := s.db.Exec(`DELETE FROM proxy_users WHERE id = ?`, id)
	return err
}

// ToggleProxyUser 切换用户启用状态
func (s *Storage) ToggleProxyUser(id int64) error {
	_, err := s.db.Exec(`UPDATE proxy_users SET enabled = CASE WHEN enabled = 1 THEN 0 ELSE 1 END WHERE id = ?`, id)
	return err
}

// VerifyProxyUser 验证代理用户凭据，返回是否有效
func (s *Storage) VerifyProxyUser(username, passwordHash string) (bool, error) {
	var storedHash string
	var enabled int
	err := s.db.QueryRow(
		`SELECT password, enabled FROM proxy_users WHERE username = ?`, username,
	).Scan(&storedHash, &enabled)
	if err != nil {
		return false, err
	}
	if enabled != 1 {
...
// HasProxyUsers 检查是否有代理用户
func (s *Storage) HasProxyUsers() (bool, error) {
	var count int
	err := s.db.QueryRow(`SELECT COUNT(*) FROM proxy_users WHERE enabled = 1`).Scan(&count)
	return count > 0, err
}
