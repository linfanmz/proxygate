...
**`proxies` 表**：代理池数据（免费+订阅）
- 代理地址、协议类型、来源（free/custom）
- 出口 IP、地理位置、延迟、质量等级（S/A/B/C）
- 使用统计、失败次数、所属订阅 ID
- 状态（active/degraded/disabled/candidate_replace）

**`subscriptions` 表**：订阅源管理
- 订阅 URL / 文件路径、名称、分组、格式
- 刷新间隔、最后拉取/成功时间、状态（active/paused）
- 代理数量、是否访客贡献

**`source_status` 表**：代理源断路器状态
- 源 URL、连续失败次数
- 状态（active/degraded/disabled）、禁用到期时间

**`proxy_sources` 表**：可配置的代理列表源
- URL、协议、分类（fast/slow）、启用状态

**`proxy_users` 表**：代理认证多用户
- 用户名、密码哈希、启用状态

**`pool_history` 表**：池子状态历史快照
- 时间戳、总数、HTTP/SOCKS5 数量、状态、质量分布

**`api_keys` 表**：外部 API 访问密钥
- 密钥哈希、名称、权限范围（read/full）、启用状态

**`audit_events` 表**：管理员操作审计日志
- 时间戳、操作者、动作、详情
...
**`config.json`** - 运行时配置（通过 WebUI 修改，首次启动自动生成）

包含：
- 池子容量配置（max_size、http_ratio、min_per_protocol）
- 延迟标准配置（max_latency_*）
- 验证配置（concurrency、timeout、validate_url）
- 健康检查配置（interval、batch_size）
- 优化配置（interval、replace_threshold）
- 地理过滤（blocked_countries、allowed_countries）
- 代理认证（proxy_auth_enabled、username、password_hash、local_bypass）
- 订阅代理配置（custom_proxy_mode、priority、probe_interval 等）
- WebUI 密码哈希

> 💡 **注意**：v0.5.0 起所有运行时配置均可通过 WebUI 修改并持久化到此文件。`.env` 文件仅作为初始默认值。
